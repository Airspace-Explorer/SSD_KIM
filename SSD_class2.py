...
path = "/data/operati123/SSD/datasets/aircraftAndBird" # train.txt와 val.txt, test.txt를 저장할 path
img_path= "/data/operati123/SSD/datasets/aircraftAndBird/training/images_8002" # 이미지가 존재하는 디렉토리 경로
...
def getInformationsFromJson(filePath):
  with open(filePath) as file:
    ...
    for annotation in oneJsonFile["annotations"]:
      ...
      else: category_id = 1 # 나머지 비행객체 :1로세팅
...
# edited here
config_file = "./mmdetection/configs/ssd/ssd300_coco.py"
checkpoint_file = './mmdetection/checkpoints/ssd300_coco_20210803_015428-d231a06e.pth'
# edited here

# Google Drive 밑에 Directory 생성. 이미 생성 되어 있을 시 오류 발생.
#if os.path.exists('./aircraft_work_dir'): shutil.rmtree("./aircraft_work_dir")
# os.mkdir("./final_class2")
...
data_root = './datasets/aircraftAndBird/'
...
cfg.data.train.dataset.ann_file = data_root + 'train.txt'
cfg.data.train.dataset.img_prefix = data_root + 'training/images_8002'

cfg.data.val.type = 'AircraftDataset'
cfg.data.val.ann_file = data_root + 'val.txt'
cfg.data.val.img_prefix = data_root + 'training/images_8002'

cfg.data.test.type = 'AircraftDataset'
cfg.data.test.ann_file = data_root + 'test.txt'
cfg.data.test.img_prefix = data_root + 'training/images_8002'
...
cfg.runner.max_epochs = 100 # edited here
...
cfg.data.samples_per_gpu = 20 # default 4
...
#cfg.dump('SSD_config_class2.py') # 수정된 config 파일 저장
